...
{
	$to = '';
	$subject = 'Envío de Formulario PARALLAX SITE';
	...
	$message .= '<tr><td valign="top" style="background-color: #ffffff;"><b>NAME:</b></td><td>' . htmlentities($_REQUEST["custom_U3792"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</td></tr>';
	...
	$message .= '<tr><td valign="top" style="background-color: #ffffff;"><b>MESSAGE:</b></td><td>' . htmlentities($_REQUEST["custom_U3798"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</td></tr>';

	$message .= '</table><br/><br/>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Formulario enviado desde el sitio web: ' . htmlentities($_SERVER["SERVER_NAME"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</div>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Dirección IP del visitante: ' . htmlentities($_SERVER["REMOTE_ADDR"],ENT_COMPAT | ENT_HTML401,'UTF-8') . '</div>';
	$message .= '</body></html>';
	$message = cleanupMessage($message);
	
	$formEmail = cleanupEmail($_REQUEST['Email']);
	$headers = 'From:  ' . "\r\n" . 'Reply-To: ' . $formEmail .  "\r\n" .'X-Mailer: Adobe Muse 6.0.751 with PHP/' . phpversion() . "\r\n" . 'Content-type: text/html; charset=utf-8' . "\r\n";
